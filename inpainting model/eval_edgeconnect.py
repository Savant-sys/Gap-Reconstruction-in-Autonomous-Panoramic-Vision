import os
import sys
# Windows: getpass.getuser() can import Unix-only 'pwd'; patch before torch so cache dir works
if os.name == "nt":
    import getpass
    def _windows_getuser():
        return os.environ.get("USERNAME") or os.path.basename(os.environ.get("USERPROFILE", "") or "user")
    getpass.getuser = _windows_getuser
    _cache = os.path.join(os.path.expanduser("~"), ".cache", "torch")
    os.environ.setdefault("TORCHINDUCTOR_CACHE_DIR", os.path.join(_cache, "inductor"))
    os.environ.setdefault("TORCH_DYNAMO_CACHE_DIR", os.path.join(_cache, "dynamo"))
import argparse
...
def main():
    ...
    # Resolve checkpoint paths: if relative and not found, try relative to this script's directory
    _script_dir = os.path.dirname(os.path.abspath(__file__))
    for attr in ("edge_ckpt", "inpaint_ckpt"):
        p = getattr(args, attr)
        ...
        if not os.path.isfile(p):
            ...
            print("Put your .pt files in the 'inpainting model' folder (e.g. checkpoints/1_edgeG_epoch50.pt)")
            print("or pass the full path: --edge_ckpt \"C:\\path\\to\\edgeG.pt\"", file=sys.stderr)
            return 1

    # Resolve dataset root: if relative, resolve from script dir so cwd doesn't matter
    root = args.root
    if not os.path.isabs(root):
        root = os.path.normpath(os.path.join(_script_dir, root))
    args.root = root
    ...
    if not os.path.isdir(img_dir) or not os.listdir(img_dir):
        ...
        print("Build the dataset first from repo root:", file=sys.stderr)
        print("  py scripts/build_inpainting_dataset.py --output-dir inpainting/waymo_data/masks", file=sys.stderr)
        print("Then run eval with: --root <path-to-that-folder> (e.g. ../inpainting/waymo_data/masks)", file=sys.stderr)
        return 1
